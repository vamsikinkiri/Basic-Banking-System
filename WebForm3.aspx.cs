...
namespace Basic_Bank
{
    public partial class WebForm3 : System.Web.UI.Page
    {
        ...
        private bool check_id(int tid)
        {
            using(SqlConnection con2=new SqlConnection(connectionString))
            {
                using (SqlCommand cmd2 = new SqlCommand("SELECT ID FROM TRANSFERS WHERE ID=@IDD", con2))
                {
                    con2.Open();
                    cmd2.Parameters.AddWithValue("@IDD", tid);
                    using (SqlDataReader readd = cmd2.ExecuteReader())
                    {
                        while (readd.Read())
                        {
                            return false;
